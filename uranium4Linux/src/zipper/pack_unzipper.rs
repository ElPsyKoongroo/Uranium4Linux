...
pub async fn unzip_pack(file_path: &str, minecraft_root: &str) -> ZipResult<()> {
    ...
    let config_result =
        fs_extra::dir::copy(TEMP_DIR.to_owned() + "config", minecraft_root, &options);
    let raw_mods_result =
        fs_extra::dir::copy(TEMP_DIR.to_owned() + "mods", minecraft_root, &options);
